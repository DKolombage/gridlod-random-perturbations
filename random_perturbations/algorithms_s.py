    ...
    def compute_combinedT(TInd):
        ...
        if model['name'] == 'check':
            ...
            if dim == 2:
                ...
                alphaT[:len(alphaT)-1] = (rPatch()[indx]-(alpha*s))/(beta-alpha)                                                # Here!!!!
            elif dim == 1:
                alphaT[:len(alphaT)-1] = (rPatch()[np.arange(len(aRefList)-1)*np.prod(NFineperEpsilon)]-(alpha*s))/(beta-alpha) # Here!!!!
